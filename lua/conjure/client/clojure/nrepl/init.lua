...
local a, action, auto_repl, client, config, debugger, eval, mapping, nvim, parse, server, str, text, ts, util =
	autoload("conjure.aniseed.core"),
	autoload("conjure.client.clojure.nrepl.action"),
	autoload("conjure.client.clojure.nrepl.auto-repl"),
	autoload("conjure.client"),
	autoload("conjure.config"),
	autoload("conjure.client.clojure.nrepl.debugger"),
	autoload("conjure.eval"),
	autoload("conjure.mapping"),
	autoload("conjure.aniseed.nvim"),
	autoload("conjure.client.clojure.nrepl.parse"),
	autoload("conjure.client.clojure.nrepl.server"),
	autoload("conjure.aniseed.string"),
	autoload("conjure.text"),
	autoload("conjure.tree-sitter"),
	autoload("conjure.util")
do
end
...
local function form_node_3f(node)
	return (
		ts["node-surrounded-by-form-pair-chars?"](node, reader_macro_pairs)
		or ts["node-prefixed-by-chars?"](node, reader_macros)
	)
end
		...
		clojure = {
			nrepl = {
				...
				eval = {
					...
					print_options = { length = 500, level = 50, right_margin = 72 },
					raw_out = false,
				},
				interrupt = { sample_limit = 0.3 },
				refresh = { after = nil, before = nil, dirs = nil },
				test = { current_form_names = { "deftest" }, runner = "clojure", call_suffix = nil, raw_out = false },
				completion = { cljs = { use_suitable = true }, with_context = true },
			},
		},
	},
})
...
local function on_filetype()
	...
	mapping.buf(
		"CljRunAlternateNsTests",
		cfg({ "mapping", "run_alternate_ns_tests" }),
